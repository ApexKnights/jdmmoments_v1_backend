...
router.post('/add-category', IsAuthenticated, async (req, res) => {
    ...
    try {
        const newcategory = await Diamond.create({
            catname,
            catimg,
            products: [],
            price,
...
router.put('/add-product/:catname', IsAuthenticated, async (req, res) => {
    ...
    try {
        const updatedCategory = await Diamond.findOneAndUpdate({ catname: categoryName }, {
            $push: {
                products: {
                    id: uuidv4(),
                    prodimg,
...
router.get('/get-categories', async (req, res) => {
    try {
        const category = await Diamond.find();
        res.status(200).json({
            success: true,
            response: category
        })
...
router.delete('/delete-category/:catname', async (req, res) => {
    ...
    try {
        const deleteCategory = await Diamond.findOneAndDelete({ catname });
        res.status(200).json({
            succes: true,
...
router.delete('/delete-product/:catname', async (req, res) => {
    ...
    try {
        const updateCategory = await Diamond.findOneAndUpdate({ catname: catname }, {
            $pull: { products: { id } }
        }, { new: true });
        res.status(200).json({
            success: true,
...
router.put('/update-price/:catname', async (req, res) => {
    ...
    try {
        const updateprice = await Diamond.findOneAndUpdate({ catname: catname }, {
            $set: { price: price }
        }, { new: true })
        res.status(200).json({
            success: true,
